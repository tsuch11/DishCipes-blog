	...
	return (
		<section className="flex flex-col items-center gap-6 max-w-7xl mx-auto px-4 py-8 md:flex-row md:items-center md:gap-10 md:px-10 md:py-12">
			...
			<div ref={leftRef} className="w-full text-center order-2 md:flex-1 md:text-right md:order-1">
				...
				<p className={`mt-3 text-sm text-brown-400 leading-relaxed md:mt-4 ${leftVisible ? 'animate-text-5' : 'opacity-0'}`}>
			...
			<div ref={rightRef} className="w-full text-center order-3 md:flex-1 md:text-left">
				<p className={`text-xs text-brown-400 mb-1 ${rightVisible ? 'animate-text-1' : 'opacity-0'}`}>-Author</p>
				<h2 className={`text-2xl font-bold text-brown-500 mb-3 ${rightVisible ? 'animate-text-2' : 'opacity-0'}`}>Teerapat N.</h2>
				<p className={`text-sm text-brown-400 leading-relaxed mb-3 ${rightVisible ? 'animate-text-3' : 'opacity-0'}`}>
				...
				<p className={`text-sm text-brown-400 leading-relaxed ${rightVisible ? 'animate-text-4' : 'opacity-0'}`}>
